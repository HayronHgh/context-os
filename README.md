...
The deterministic control remains frozen at [`v0.1.2`](https://github.com/HayronHgh/context-os/tree/v0.1.2), and M4 experiment inputs are pinned to `aa59f4d`. The v0.2 development line now adds a strict zero-mutation execution preflight: current recovery sources are revalidated before a distinct immutable `ExecutablePlan` may be emitted. Transformation, execution, context mutation, artifact creation, and memory promotion remain absent.
